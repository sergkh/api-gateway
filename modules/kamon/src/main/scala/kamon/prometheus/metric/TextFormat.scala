...
object TextFormat extends SerialisationFormat[String] {
  ...
  override def format(snapshot: Seq[PrometheusMetricFamily]): String = {
    snapshot.map { family ⇒
  ...
  override def parse(source: String): Seq[PrometheusMetricFamily] = {
  ...
  private[metric] object ParseHelper extends JavaTokenParsers {
    ...
    def metricFamily: Parser[PrometheusMetricFamily] = opt(helpLine) ~ typeLine ~ rep(metricLine) ^^ {
    ...
    private def evaluateMetricFamily(name: String, prometheusType: PrometheusType, help: Option[String],
                                     metricLines: Seq[MetricLine]): PrometheusMetricFamily = {
      ...
      PrometheusMetricFamily(name, prometheusType, help, metrics)
    }

    /** Parses an entire snapshot, a sequence of metric familiies. */
    def snapshot: Parser[Seq[PrometheusMetricFamily]] = rep(metricFamily)
  }
}
