...
import kamon.prometheus.converter.SnapshotConverter._
import kamon.prometheus.metric.{Metric, MetricValue, PrometheusMetricFamily, PrometheusType}
...
class SnapshotConverter(settings: PrometheusSettings) {
  ...
  def apply(tick: TickMetricSnapshot): Seq[PrometheusMetricFamily] = {
  ...
  private def makeCounterMetricFamily(snapshots: Iterable[MetricSnapshot], timestamp: MilliTimestamp) = {
    ...
    PrometheusMetricFamily(Mungers.asMetricName(snapshots.head.name + suffix), PrometheusType.Counter, None, metrics.toSeq)
  }

  /** Builds a metric family corresponding to a Kamon histogram. */
  private def makeHistogramMetricFamily(snapshots: Iterable[MetricSnapshot], timestamp: MilliTimestamp) = {
    assert(snapshots.nonEmpty, "A metric family requires at least one member")
    ...
    // TODO: handle help
    PrometheusMetricFamily(Mungers.asMetricName(snapshots.head.name + suffix), PrometheusType.Histogram, None, metrics.toSeq)
  }

  /** Builds a metric family corresponding to a Kamon min-max counter. */
  private def makeMinMaxCounterMetricFamily(snapshots: Iterable[MetricSnapshot], timestamp: MilliTimestamp) = {
    assert(snapshots.nonEmpty, "A metric family requires at least one member")
  ...
  private def makeGaugeMetricFamily(snapshots: Iterable[MetricSnapshot], timestamp: MilliTimestamp) = {
    assert(snapshots.nonEmpty, "A metric family requires at least one member")
  ...
  /** Constructs a list of metric families for an arbitrary entity recorder.  Since these may have more than one
    ...
    */
  private def makeArbitraryMetricFamilies(snapshots: Iterable[MetricSnapshot], timestamp: MilliTimestamp): Seq[PrometheusMetricFamily] = {
    ...
    groupedInstrumentSnapshots.map { case (key, data) ⇒
      ...
      PrometheusMetricFamily(familyName, prometheusType, None, metrics.toSeq)
    }.toSeq
  }
}
